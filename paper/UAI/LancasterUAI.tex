...
\end{align*}
By assumption, $\mathbb{P}_{XYZ} =\mathbb{P}_{XY}\mathbb{P}_{Z}$ and thus the expectation operator also factorises similarly. As a consequence, $C_{XYZ}=0$. Indeed, for any $A \in HS(\mathcal{F_Z}, \mathcal{F}_X \otimes \mathcal{F_Y})$
\begin{align*}
...
\vspace{-0.7cm}
where the penultimate line follows from the linearity of $A$ and the Bochner integrability of $\bar{\phi}_Z(Z)$, which in turn follows from the conditions required for $\mu_Z$ to exist. We conclude that $C_{XYZ} = \mathbb{E}_{XYZ} \bar{C}_{XYZ} = 0$.
