...
We will assume that kernels $k,l,m$ are characteristic  \cite{sriperumbudur2011universality}\cite{sriperumbudur2010hilbert} and bounded. We describe some notation relevant to the kernel $k$; similar notation holds for $l$ and $m$.


Recall that $\mu_X := \mathbb{E}_X k(X,\cdot) \in \mathcal{H}_k$ is the  mean embedding \cite{smola2007hilbert} of the random variable $X$. The  estimate of the mean embedding is $\tilde{\mu}_X = \frac{1}{n}\sum_{i=1}^n k(X_i,\cdot)$, we  remark that $\tilde{k}(x,x') = \langle k(x,\cdot)-\tilde{\mu}_X, k(x',\cdot)-\tilde{\mu}_X \rangle$ is a kernel. $\bar{k}(x,x') = \langle\phi_X(x)-\mu_X,\phi_X(x')-\mu_X\rangle$ is a kernel. 

% TODO if you want this put it to appendix  
...
% TODO irelevant for details can briefly discusses in intro 
% \subsection{Hypothesis testing using the mean embedding}
% 
% The idea of injectively embedding measures into a Hilbert space can be exploited to design statistical tests of properties of one or more distributions. For example, the Maximum Mean Discrepancy (MMD) \cite{gretton2012kernel} two-sample test is motivated by the following: suppose we are given samples $\{X_i\}_{i=1}^n$ and $\{Y_j\}_{j=1}^m$ drawn \emph{i.i.d.} from distributions $\mathbb{P}$ and $\mathbb{Q}$ respectively. If our kernel is characteristic, then $\mu_\mathbb{P} = \mu_\mathbb{Q} \iff \mathbb{P} = \mathbb{Q}$. We can construct a statistical test using an estimator of the quantity $\|\mu_\mathbb{P} - \mu_\mathbb{Q} \|^2$ as our test statistic, the null hypothesis of which is that $\mathbb{P} = \mathbb{Q}$. The quantiles of the null distribution can be estimated and therefore we can set consistent thresholds.
% 
...
\paragraph{Lancaster interaction measure}
...
\subsection{Time series, mixing}
As outlined in the main results section, there are various formalisations of memory or 'mixing' of a random process \cite{doukhan1994mixing}\cite{bradley2005basic}\cite{dedecker2007weak}; of relevance to this paper are the following two:
...
\paragraph{Mixing assumptions}

TODO: Collect out mixing assumptions, get common denominator and paste here. Refer to them as mixing assumptions when possible.


% TODO this is not necessary to prove Theorem 2, altermatively you can put  O(n^{-\frac{1}{2}}) notation in the Theorem 2.
...
We call $nV_n$ a \emph{normalised} V-statistic. We call $f$ the \emph{core} of $V$ and we say that $f$ is \emph{degenerate} if, for any $s_1$, $\mathbb{E}_{S_2 \sim \mathbb{P}}[f(s_1,S_2)] = 0$, in which case we say that $V$ is a \emph{degenerate V-statistic}.

V-statistics are useful to us because their properties are well understood, and many kernel test statistics can be viewed as normalised V-statistics which, under the null hypothesis, are degenerate. If moreover the test statistics diverge under the alternative hypothesis, the test would be consistent. In the Theorem 2 we prove that, under the null hypothesis, the Lancaster statistic is asymptotically a degenerate V-statistic.
