...
In this section we discuss the wild bootstrap and provide consistency and Type I error results.
...
Given the statistic $nV(h,\mathcal{S}_n)$, \citet{leucht2013dependent} tells us that a random vector $W$ of length $n$ can be drawn such that the bootstrapped statistic\footnote{Note that for fixed $\mathcal{S}_n$, $nV_b(h,\mathcal{S}_n)$ is a random variable through the randomness introduced by $W$}
\[nV_b(h,\mathcal{S}_n)=\frac{1}{n}\sum_{i,j}W_{i}h(S_i,S_j)W_{j}\]
...
\[nV_b = \frac{1}{n}\left(W^\intercal\left( \overline{\left( \bar{K} \circ \bar{L}\right) }\circ \bar{M} \right)W\right)\]
