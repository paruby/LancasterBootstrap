...
\end{align}
 ...
 Given an i.i.d. sample $(X_i,Y_i,Z_i)_{i=1}^n$, the norm of the mean embedding $\mu_L$ can be empirically estimated using empirically centered kernel matrices. For example, for the kernel $k$ with kernel matrix $K_{i,j} = K(X_i,X_j)$, the empirically centered kernel matrix $\tilde{K}$ is given by
\[
...
Suppose that $(X_i)$ is $\beta$-mixing with coefficients $\beta(m)$ satisfying $\sum_{m=1}^{\infty}\beta(m)^{\frac{\delta}{2+\delta}}<\infty$ for some $\delta > 0$ and that $k$ is a bounded kernel on $\mathcal{X}$. Then $\|\hat\mu_X - \mu_X\|_k = O_P(n^{-\frac{1}{2}})$
...
The key idea is to note that we can rewrite $n\|\hat \mu_L\|^2$ in terms of the population centred kernel matrices $\overline{K}$, $\overline{L}$ and $\overline{M}$. Each of the resulting terms can in turn be converted to an inner product between quantities of the form $\hat\mu - \mu$, where $\hat\mu$ is an empirical estimator of $\mu$, where the $\mu$ are mean embeddings and covariance operators.

By applying Lemma \ref{lemma:hilbertCLT} to show that the $\hat\mu - \mu$ converge to 0 sufficiently quickly, we show that most of these terms converge in probability to 0, with the residual terms equaling $n\| \hat \mu^{(Z)}_{L,2} \|^2$.
\end{proof}
...
As mentioned in the previous section,  $\|\hat \mu^{(Z)}_{L,2}\|^2$ is a V-statistic \citep{serfling2009approximation}. A V-statistic   of a 2-argument, symmetric function $h$ given  observations $\mathcal{S}_n = \{S_1,\ldots,S_n\}$  is:
...
The condition on the core that is of crucial importance to this paper is that it must be degenerate. Theorem \ref{theorem:degenerate-kernel} justifies the use of the wild bootstrap.

Given the statistic $nV(h,\mathcal{S}_n)$, \citet{leucht2013dependent} tells us that a random matrix $W$ can be drawn such that the bootstrapped statistic\footnote{Note that for fixed $\mathcal{S}_n$, $nV_b(h,\mathcal{S}_n)$ is a random variable through the randomness introduced by $W$}
\[nV_b(h,\mathcal{S}_n)=\frac{1}{n}\sum_{i,j,p,q}W_{ij}h(S_j,S_p)W_{pq}\]
is distributed according to the null distribution of $nV$. 
