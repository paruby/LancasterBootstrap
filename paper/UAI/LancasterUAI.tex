...
In time series setting this reasoning does not hold. It's difficult to study asymptotic disquisition of $n \|\hat \mu_L\|^2 $ for non i.i.d.  data and permutation bootstrap fails as showed in [cite something]. to construct a test in this setting we will use asymptotic and bootstrap results for $\tau$-mixing. To do so first we need to simplify problem to studying 
\[
\| \hat \mu_{L,2} \|^2 =\frac{1}{n^2}\left(\overline{K}\circ\overline{L}\circ\overline{M}\right)_{++}
\]
where e.g. 
\[
...
\begin{theorem}
 Under the null, $\lim_{n \to \infty} ( \| \hat \mu_{L,2} \|^2 -\|\hat \mu_L\|^2 ) =0 $ in probability.
\end{theorem}
The statistic $\| \hat \mu_{L,2} \|^2$ is much easier to study under non-i.i.d.  assumption. Indeed it can be written as a V-statics 
\[ 
V_n = \frac{1}{n^2} \mathlarger{\sum}_{1\leq i,j \leq n} \overline{k} \otimes \overline{l}\otimes \overline{m} (S_i,S_j)
\]
where  $S_i = (X_i,Y_i,Z_i)$. The crucial observation is that
\[
 h = \overline{k} \otimes \overline{l}\otimes \overline{m}
\]
is well behaved in the following sense 
\begin{theorem}
 $h$ is bounded symmetric Lipschitz contentious kernel, which is degenerate under the null hypothesis i.e $ < Eh(S_i,\cdot),h(s,\cdot) > =0$ (  Paul i think this is easily done in RKHS using mean embedding def of Lancaster, somehow similar to what you did at the end of the proof of th 1.)
\end{theorem}
