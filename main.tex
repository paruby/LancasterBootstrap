...
\kacper{Can't you use clt for $\sqrt n \tilde{C}_{XY}$, $ \sqrt n( \tilde{\mu}_Y - \mu_Y)$ , $\sqrt n (\tilde{\mu}_X - \mu_X)$?  You would have 
\[
 \frac{1}{\sqrt n} \langle \sqrt n \tilde{C}_{XY},\sqrt n (\tilde{\mu}_X - \mu_X) \otimes \sqrt n (\tilde{\mu}_Y - \mu_Y)\rangle.
\]
Currently you use result for HSIC, I thought that idea is to use only CLT (LLN) in Hilbert space and have proof to which you plug in the most up to date CLT.} 
\end{claimproof}
